...
NOTE: THIS VERSION INTENDS TO USE 3-DEEP NEIGHBOURS BUT IT DOES NOT WORK THAT GOOD. MAYBE REVISE IT IN THE FUTURE
...
class Solution:
    ...
    def __init__(self, target, fig):
        ...
        self.score_list = [set() for _ in range(0, 97)]  # a list to quickly access to the lowest scoring path
    ...
    def fill_node(self, r, c):
        ...
        nn = ee = ss = ww = ne = nw = se = sw = False  # same as above, but for 3rd order neighbours
        ...
        if not self.second_phase:  # first phase
            if not north_edge and self.target[r - 1][c] == 1:  # check neighbours north
                ...
                if r > 1 and self.target[r - 2][c] == 1:  # 2nd order (2 squares away) neighbour
                    score += 1
                    nn = True
                    if r > 2 and self.target [r - 3][c] == 1: # 3rd order
                        score += 1
            if not south_edge and self.target[r + 1][c] == 1:  # check neighbours south
                ...
                if r < self.height - 2 and self.target[r + 2][c] == 1:
                    score += 1
                    ss = True
                    if r < self.height - 3 and self.target [r + 3][c] == 1: # 3rd order
                        score += 1
            if not west_edge and self.target[r][c - 1] == 1:  # check neighbours west
                ...
                if c > 1 and self.target[r][c - 2] == 1:
                    score += 1
                    ww = True
                    if c > 2 and self.target [r][c - 3] == 1: # 3rd order
                        score += 1
            if not east_edge and self.target[r][c + 1] == 1:  # check neighbours east
                ...
                if c < self.width - 2 and self.target[r][c + 2] == 1:
                    score += 1
                    ee = True
                    if c < self.width - 3 and self.target [r][c + 3] == 1: # 3rd order
                        score += 1
            ...
            if (nn or ne) and r >= 2 and c < self.width - 1 and self.target[r - 2][c + 1] == 1:
                score += 1
            if (ne or ee) and r >= 1 and c < self.width - 2 and self.target[r - 1][c + 2] == 1:
                score += 1
            if (ee or se) and r < self.height - 1 and c < self.width - 2 and self.target[r + 1][c + 2] == 1:
                score += 1
            if (se or ss) and r < self.height - 2 and c < self.width - 1 and self.target[r + 2][c + 1] == 1:
                score += 1
            if (ss or sw) and r < self.height - 2 and c >= 1 and self.target[r + 2][c - 1] == 1:
                score += 1
            if (sw or ww) and r < self.height - 1 and c >= 2 and self.target[r + 1][c - 2] == 1:
                score += 1
            if (ww or nw) and r >= 1 and c >= 2 and self.target[r - 1][c - 2] == 1:
                score += 1
            if (nw or nn) and r >= 2 and c >= 1 and self.target[r - 2][c - 1] == 1:
                score += 1
                    ...
                    for n1 in list_of_neighbours1:
                        ...
                        for n2 in list_of_neighbours2:
                            ...
                            for n3 in list_of_neighbours3:
                                if n3 not in list_1 and n3 not in deleted_blocks:
                                    list_1.append(n3)
                                    if n3 not in list_2:
                                        list_2.append(n3)
        else:
